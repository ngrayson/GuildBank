...
const itemManager = require('../../GuildHall/itemManager.js');

let name = `newItem`;

module.exports.help = {
	name: name,
	description: 'creates a new base item',
	format: `!${name} @user <numberAmount>`,
	note: 'if rarity is floating point, it will be rounded'
}
...
module.exports.run = async(bot, message, args) => {
	...
	try{
		if(args.length < 1 ) throw `newItem requires at least 1 argument (you provided ${args.length})`;
		let res = await itemManager.newBaseItem(args[0]);
		msg.edit(res.name);
